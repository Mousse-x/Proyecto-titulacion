            ...
            {loading ? (
              ...
              return (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => navigate(`/university/index?university_id=${item.id}`)}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter' || event.key === ' ') {
                      event.preventDefault();
                      navigate(`/university/index?university_id=${item.id}`);
                    }
                  }}
                  title={`Ver informacion de transparencia de ${item.full_name || item.name}`}
                >
